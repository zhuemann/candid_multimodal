...
def train_image_text_segmentation(config, args , batch_size=8, epoch=1, dir_base = "/home/zmh001/r-fcb-isilon/research/Bradshaw/", n_classes = 2):
    ...
    LR = 8e-4 #5e-5 #5e-5 was lr for contextualnet runs #8e-5  # 1e-4 was for efficient #1e-06 #2e-6 1e-6 for transformer 1e-4 for efficientnet
    #LR = 5e-4
    N_EPOCHS = epoch
    N_CLASS = n_classes
    #LR = 1e-4
...
#        ])
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=127800, eta_min=8e-6, last_epoch=-1, verbose=False)
