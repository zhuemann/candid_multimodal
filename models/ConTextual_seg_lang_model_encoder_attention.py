...
class Attention_ConTEXTual_Lang_Seg_Model(torch.nn.Module):
    def __init__(self, lang_model, n_channels, n_classes, bilinear=False):
        ...
        self.lang_attn3_encoder = LangCrossAtt(emb_dim=256)
        self.lang_proj2_encoder = nn.Linear(lang_dimension, 128)
        self.lang_attn2_encoder = LangCrossAtt(emb_dim=128)
        self.lang_proj1_encoder = nn.Linear(lang_dimension, 64)
        self.lang_attn1_encoder = LangCrossAtt(emb_dim=64)
    ...
    def forward(self, img, ids, mask, token_type_ids):
        ...
        lang_rep1_encoder = self.lang_proj1_encoder(lang_rep)
        x1 = self.lang_attn1_encoder(lang_rep=lang_rep1_encoder, vision_rep=x1)

        x2 = self.down1(x1)

        lang_rep2_encoder = self.lang_proj2_encoder(lang_rep)
        x2 = self.lang_attn2_encoder(lang_rep=lang_rep2_encoder, vision_rep=x2)

        x3 = self.down2(x2)

        lang_rep3_encoder = self.lang_proj3_encoder(lang_rep)
        x3 = self.lang_attn3_encoder(lang_rep=lang_rep3_encoder, vision_rep=x3)

        x4 = self.down3(x3)

        lang_rep4_encoder = self.lang_proj4_encoder(lang_rep)
        x4 = self.lang_attn4_encoder(lang_rep=lang_rep4_encoder, vision_rep=x4)
